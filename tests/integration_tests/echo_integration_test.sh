...
echo "Testing echo server with GET request"
curl "http://localhost:8080/echo" -isS -o response.out



# Test that the response from the web server is valid
if grep -q "200 OK" response.out;
then
	echo "Expected response received from the web server"
else
	echo "Unexpected response received from web server"
