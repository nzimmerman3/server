...
std::vector<std::shared_ptr<NginxConfigStatement>> NginxConfig::getStatementsFor(std::string directive) const {
  std::vector<std::shared_ptr<NginxConfigStatement>> stmts;

  for (const auto& statement : statements_) {
    if (statement->tokens_.size() > 0 && statement->tokens_[0] == directive) {
